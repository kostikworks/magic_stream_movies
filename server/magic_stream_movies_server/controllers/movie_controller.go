...
func GetMovie(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if movieID == "" {
			c.IndentedJSON(http.StatusBadRequest, gin.H{"err": "movie Id is required"})
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusNotFound, gin.H{"err": "movie not found"})
			return
		}
...
func AddMovie(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// create movie variable to store movie data and unmarshal json
		var movie models.Movie
		if err := c.ShouldBindJSON(&movie); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"err": "invalid input"})
			return
		}
		// validate json fields (required etc)
...
// add movie review by AdminReview
...
func AdminReviewUpdate(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "failed to fetch user role"})
			return
		}

		if role != "ADMIN" {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "user must be a part of the ADMIN role"})
			return
		}
...
func GetRecommendedMovies(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "failed to retrieve userId"})
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to retrieve user's genres"})
			return
		}
...
func GetGenres(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "failed to retrieve genres collection"})
			return
		}
